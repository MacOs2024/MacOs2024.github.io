...
# Общие JS-хелперы: P() понимает запятую как разделитель и селект единиц id+'_u'
HELPERS_JS = r"""
...
var u=$(id+'_u');return u?n*N(u.value):n;}
...
function out(h){var r=$('res');r.innerHTML=h;r.classList.add('on');}
function err(m){var r=$('res');r.innerHTML='<div class="rerr">'+m+'</div>';r.classList.add('on');}
...
<!-- Яндекс.Метрика: код счётчика вставить сюда на Этапе 2 -->
<style>@CSS@</style>
</head>
<body>
...
def field_html(f):
    ...
    if f.get("units"):
        ...
        unit_html = '<select id="%s_u" class="u" aria-label="Единицы">%s</select>' % (fid, o)
    elif f.get("unit"):
...
def index_html(calcs):
    ...
    return (INDEX.replace("@T@", esc(t)).replace("@D@", esc(d)).replace("@CSS@", CSS)
...
- `*.html` — страницы калькуляторов (25 шт.)
