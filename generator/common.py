...
def page_html(c, all_by_slug):
    ...
    if notice:
        ...
        calcblock = (
            ...
            % (c["name"], fields))
        # Тема в заголовке, а не только в title: h2 — второй по весу сигнал
        # релевантности, а «Как считается» само по себе не говорит ни о чём.
        # Тема ставится первой: так ключевые слова в начале строки, и по-русски
        # это грамматически безопасно — двоеточие снимает вопрос падежа.
        # Заголовок FAQ намеренно оставлен общим: третий повтор названия на
        # одной странице читался бы как переспам.
        # Если в названии уже есть двоеточие («Закон Ома: ток, напряжение…»),
        # берём часть до него: иначе в заголовке окажется два двоеточия подряд.
        topic = esc((c.get("h2_topic") or c["name"]).split(":")[0].strip())
...
<p>Проект ведёт <b>Максим</b> — независимый разработчик. Профессия автора не связана с электротехникой, и это стоит сказать прямо: <b>сайт не претендует на заключение дипломированного инженера</b>. Вместо личного авторитета — прозрачная процедура: каждая формула сверяется с первоисточником, покрывается эталонными тестами и получает открытый статус проверки, который виден на странице каждого калькулятора.</p>
<p>Часть работы по подбору источников, написанию тестов и правке формул выполняется с помощью ИИ-инструментов. Это не скрывается: статус «Сверено с источником и тестами» означает сверку агентом по документу, а не подпись практикующего инженера — разница объяснена ниже.</p>
